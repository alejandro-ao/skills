...
GROQ_API_KEY=your_key          # Groq Whisper API

# Required for content generation (descriptions, timestamps)
OPENAI_API_KEY=your_key        # OpenAI GPT-4

# Optional - Bunny.net CDN uploads
BUNNY_LIBRARY_ID=your_id
BUNNY_ACCESS_KEY=your_key
BUNNY_COLLECTION_ID=your_id
...
For YouTube uploads, run OAuth2 setup (no env vars needed):
```bash
...
#### Enhance Audio
Improve audio quality using Resemble AI (requires REPLICATE_API_TOKEN).
```bash
...
#### Generate Transcript
Create VTT captions using Whisper (requires GROQ_API_KEY).
```bash
video-tool video transcript -i video.mp4 -o transcript.vtt
```

#### Generate Timestamps
Create chapter markers (requires OPENAI_API_KEY).
```bash
...
Create video description with optional links (requires OPENAI_API_KEY).
```bash
...
#### Bunny.net Uploads
Upload to Bunny.net CDN (requires BUNNY_* env vars).
```bash
